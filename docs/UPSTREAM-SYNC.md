...
- **Sync when we intend to cut a fork release**, not on every upstream
  ...
  upstream.
- **Version scheme:** plain semver, counting this repository's own releases.
  The fork previously encoded its platform in the version, first as build
  metadata (`2.9.0+linux.2`) and then as a prerelease tag (`2.9.0-linux.2`).
  Both break the in-app updater, which compares semver: build metadata is
  ignored in precedence, so `+linux.2` and `+linux.3` compare equal, and a
  prerelease sorts *below* the bare version, so every release would look like
  a downgrade of one that is never published. Platforms are per-platform
  artifacts of one version instead. The upstream release a version is based
  on belongs in the release notes, not in the version.
...
- **Version files** (`package.json`, `src-tauri/Cargo.toml`,
  ...
  same three lines every release. Never take either side. Recompute: the
  fork's own next plain-semver version, via `scripts/bump-version.sh`.
...
The workflow drafts a release from both platforms' bundles. Publishing that
draft is what writes `latest.json`, so an update only reaches installed copies
once the notes are written and the draft goes live.
