...
/// Held for the length of a `get_or_refresh`, so two callers after the same
/// cache take turns and the second one finds what the first stored.
    ...
    T: Serialize + DeserializeOwned,
{
    ...
    // ponytail: one lock for every cache; split it per name if a slow download
    // ever holds up an unrelated refresh noticeably.
    let _refreshing = REFRESHING.lock().unwrap_or_else(|e| e.into_inner());
