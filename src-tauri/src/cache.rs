...
/// One lock per cache name, held for the length of a `get_or_refresh`, so two
/// callers after the same cache take turns and the second one finds what the
/// first stored — while a slow download of one cache never blocks another.
    ...
    T: Serialize + DeserializeOwned,
{
    ...
    match result {
        ...
        Ok(Fetched::NotModified) => match cached {
            ...
            None => {
                let warning = format!("{name}: server sent Not-Modified, but there is no cached copy");
                report(name, None, Source::Fallback, Some(warning), None)
            }
        },
        Err(e) => match cached {
            Some(c) => {
                let warning = format!("{name}: refresh failed: {e}. Showing cached data.");
