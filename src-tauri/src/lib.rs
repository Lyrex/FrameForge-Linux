...
async fn fetch_item_list(state: State<'_, AppState>, force: Option<bool>) -> Result<usize, String> {
    ...
    let (result, _, warning) = tauri::async_runtime::spawn_blocking(move || {
...
/// this runs once per catalogued image on every startup.
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            match tokio::fs::read(&path).await {
                ...
                Ok(data) if !looks_like_image(&data) => {
