...
/// Caches the old layout kept beside the user's real files. Deleted rather than
/// moved: every one of these refills itself on the next fetch.
...
fn copy_then_delete(src: &Path, dst: &Path) -> io::Result<()> {
    ...
    // be on the platter before then — the directory entry included, or a power
    // loss right after the unlink loses the file despite its synced contents.
