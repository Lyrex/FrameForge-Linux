...
struct Source {
    /// Absent when the server confirmed the copy we already had, and when the
...
/// source that was never there. Only All.json is mandatory on a first build; the
/// other three degrade to their empty defaults so a syndicate or relic outage
/// does not cost a new user the item list.
