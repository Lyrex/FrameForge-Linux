...
///
/// A price is a fact about a trade that happened, so it keeps for the session.
/// An absent price only describes the order book at the moment of the lookup —
/// an item listed an hour later would otherwise stay unpriced until a restart.
fn price_entry_live(price: Option<u32>, age: Duration) -> bool {
    price.is_some() || age < NEGATIVE_PRICE_TTL
}
