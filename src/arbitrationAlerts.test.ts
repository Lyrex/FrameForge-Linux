...
test("an hour whose lead window was slept through still alerts", () => {
  const entries = [entry(10 * HOUR)];
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, [], 10 * HOUR)), ["SolNode1"]);
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, [], 10 * HOUR + 30 * 60)), ["SolNode1"]);
});

test("an hour too far gone to join stays quiet", () => {
  const entries = [entry(10 * HOUR)];
  const lastUseful = 11 * HOUR - MIN_REMAINING_SECS;
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, [], lastUseful - 1)), ["SolNode1"]);
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, [], lastUseful)), []);
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, [], 11 * HOUR)), []);
});

test("an hour alerted before it started is not alerted again once it runs", () => {
  const entries = [entry(10 * HOUR)];
  const fired = persistAll(dueAlerts(entries, ["SolNode1"], 10, [], 10 * HOUR - 600));
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, fired, 10 * HOUR + 60)), []);
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, fired, 10 * HOUR + 30 * 60)), []);
});
...
test("the same node alerts again on its next occurrence", () => {
  ...
  const fired = persistAll(dueAlerts(entries, ["SolNode1"], 10, [], 10 * HOUR - 60));
...
test("keys outlive the start they name, then are pruned", () => {
  ...
  assert.deepEqual(dueAlerts(entries, ["SolNode1"], 10, stale, 10 * HOUR + 60).kept, stale);
  assert.deepEqual(dueAlerts(entries, ["SolNode1"], 10, stale, 20 * HOUR).kept, [`${40 * HOUR}@SolNode1`]);
});

test("a key that parses to no number is dropped rather than kept forever", () => {
  const stale = ["Infinity@SolNode1", "garbage", "@SolNode1"];
  assert.deepEqual(dueAlerts([], ["SolNode1"], 10, stale, 20 * HOUR).kept, []);
});

test("favoriting mid-window alerts on the next tick", () => {
  const entries = [entry(10 * HOUR)];
  const quiet = dueAlerts(entries, [], 10, [], 10 * HOUR - 300);
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 10, persistAll(quiet), 10 * HOUR - 240)), ["SolNode1"]);
});
...
test("lead is clamped into range, so a zero or negative lead cannot fire late", () => {
  ...
  const entries = [entry(10 * HOUR)];
  assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], 0, [], 10 * HOUR - 120)), []);
...
test("the shortest lead is still offered on several consecutive ticks", () => {
  ...
  for (let t = 0; t * tickSecs < windowSecs; t++) {
    ...
    assert.deepEqual(due(dueAlerts(entries, ["SolNode1"], MIN_LEAD_MINS, [], now)), ["SolNode1"]);
    offered++;
  }
...
test("only occurrences inside the window join the fired state", () => {
  ...
  assert.deepEqual(persistAll(dueAlerts(entries, ["SolNode1"], 10, [], 10 * HOUR - 60)),
    [`${10 * HOUR}@SolNode1`]);
});

test("a raised alert joins the persisted state", async () => {
  const entries = [entry(10 * HOUR)];
  assert.deepEqual(await runAlertPass(entries, ["SolNode1"], 10, [], 10 * HOUR - 600, raiseAll),
    [`${10 * HOUR}@SolNode1`]);
});

test("an alert that could not be raised is not recorded, and comes back", async () => {
  const entries = [entry(10 * HOUR)];
  assert.equal(await runAlertPass(entries, ["SolNode1"], 10, [], 10 * HOUR - 600, raiseNone), null);

  const seen: string[] = [];
  await runAlertPass(entries, ["SolNode1"], 10, [], 10 * HOUR - 540, async e => {
    seen.push(e.node_id);
    return true;
  });
...
test("only the occurrences that were raised are recorded", async () => {
  const entries = [entry(10 * HOUR, "SolNode1"), entry(10 * HOUR, "SolNode2")];
  assert.deepEqual(
    await runAlertPass(entries, ["SolNode1", "SolNode2"], 10, [], 10 * HOUR - 600,
...
test("a pass that changes nothing reports nothing to save", async () => {
  ...
  assert.equal(await runAlertPass(entries, ["SolNode1"], 10, fired, 10 * HOUR - 300, raiseAll), null);
});

test("a pass that only prunes still reports the pruned state", async () => {
  assert.deepEqual(await runAlertPass([], [], 10, [`${8 * HOUR}@SolNode1`], 10 * HOUR, raiseAll), []);
});
